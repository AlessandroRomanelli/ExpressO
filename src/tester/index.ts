...
const compare = (
  ...
  extractorFn: (x: OpenAPIV3.Document) => string[],
): CoverageReport => {
...
const getResponses = (x: OpenAPIV3.Document): string[] =>
  Object.keys(x.paths).flatMap((pattern) =>
...
const getParameters = (x: OpenAPIV3.Document): string[] =>
  Object.keys(x.paths).flatMap((url) =>
    (Object.keys(x.paths[url] || {}) as HTTPMethod[]).flatMap((endpoint) => {
      ...
      return (parameters as OpenAPIV3.ParameterObject[]).map((parameter) => `${url}#${endpoint}#${parameter.name}`);
    }),
  );
