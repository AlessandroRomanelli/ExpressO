...
const generateCoverageBar = (coverage: number): string => `${'\u2588'.repeat(Math.round(CLIWidth*coverage))}${'\u2591'.repeat(Math.round(CLIWidth*(1-coverage)))}`
...
const generateTabularResults = (
  ...
  iterates = ['pattern', 'method']): string =>
  table(_.sortBy(results.map(mappingFn), iterates).map(x => Object.values(x).reverse()), tableConfig).trimEnd()
...
${
  ...
  ] as [string, string[]][]).map(([title, res]) => generateResults(title, res, parseEndpoint)).join("\n\n")
}`

export const generateReport = (results: ComparisonResults) => `Results for OpenAPI specification comparison between the following files:
  - ${results.custom}
  - ${results.generated}
...
${
  ...
  ] as [string, CoverageReport][]).map(([title, res]) => generateReportSubtype(res, title)).join("\n\n")
}`