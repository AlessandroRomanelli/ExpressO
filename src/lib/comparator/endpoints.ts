...
export class Endpoint {
  ...
  normalMatch = (other: Endpoint): boolean => this.normalizedPath === other.normalizedPath && this.method === other.method
...
export const compareEndpoints = (customSpec: OpenAPIV3.Document, generatedSpec: OpenAPIV3.Document): CoverageReport => {
  ...
  let [missing, matched] = _.partition(custom, (x) =>
    !_.find(generated, (y) => x.match(y)));
  let additional = generated
    .filter((x) => !_.find(custom, (y) => x.match(y)));
