  ...
  Object.keys(x.paths).flatMap((pattern) => {
    ...
    return (Object.keys(patternObj) as HTTP_METHOD[])
      ...
      .flatMap((method) => {
        ...
        return ([] as OpenAPIV3.ParameterObject[]).concat(patternParameters, parameters).map((parameter) => {
          return new Parameter(path.normalize(basePath + pattern), method, parameter.name, parameter.in);
        });
      });
  });

export const compareParameters = (
  ...
  let [missing, matched] = _.partition(custom, (x) => !_.find(generated, (y) => x.matchParameter(y)));
  let additional = generated.filter((x) => !_.find(custom, (y) => x.matchParameter(y)));
  ...
  const partiallyMatched = missing
    ...
    .map((x) => {
      ...
      return new Parameter(
        reinsertParams(x.normalizedPath, pathParams),
        x.method,
        `${x.parameter}|${other.parameter}`,
        x.where,
      );
    });
