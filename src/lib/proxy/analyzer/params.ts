...
const paramToSpecification = (paramName: string, location: 'path' | 'query', required = false): OpenAPIV3.ParameterObject => ({
  in: location,
  name: paramName,
...
export const mineFunctionParamNames = (fn: any): string[] => {
  ...
  return find(fn, `MemberExpression[object.object.name='${reqName}'][object.property.name='query']`)
